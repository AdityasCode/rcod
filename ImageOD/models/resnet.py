...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_feature=False, return_feature_list=False):
        ...
        features = torch.flatten(out, 1)        # penultimate features
        logits = self.linear(features)          # final classifier

        if return_feature or return_feature_list:
            # OpenOOD expects (logits, features)
...
def resnet18(num_classes=10):
    net = models.resnet18(weights=None)   # no pretrained weights here
